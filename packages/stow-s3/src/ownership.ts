...
export class StowOwnershipError extends Error {
  ...
  constructor(readonly dataDir: string, reason: string) {
 ...
 */
function isAncestorOrSelf(candidate: string, protectedDir: string): boolean {
 ...
 */
function assertNotProtectedPath(dataDir: string): void {
  const target = resolve(dataDir);
  const home = homedir();
  if (isAncestorOrSelf(target, home)) {
    throw new StowOwnershipError(
      dataDir,
      "it is your home directory or an ancestor of it. Pass a subdirectory stow created.",
    );
  }
  if (isAncestorOrSelf(target, process.cwd())) {
    throw new StowOwnershipError(
      dataDir,
      "it is the current working directory or an ancestor of it. Pass a subdirectory stow created.",
    );
  }
}
 ...
 */
export async function resetOwnedData(dataDir: string): Promise<void> {
  ...
  if (!(await exists(join(target, STOW_OWNER_MARKER)))) {
    throw new StowOwnershipError(
      dataDir,
      `it has no ${STOW_OWNER_MARKER} marker, so stow did not create it. ` +
