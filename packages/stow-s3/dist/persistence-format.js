import { BROWSER_PERSISTENCE_FORMAT_VERSION, BrowserPersistenceError, } from "./browser-types.js";
export function normalizeSnapshot(value, fallbackGeneration = 0) {
    if (!isRecord(value)) {
...
export function applyChanges(snapshot, changes) {
    ...
    const objects = new Map(snapshot.objects.map((object) => [objectKey(object.bucket, object.key), normalizeObject(object)]));
...
function deleteBucketObjects(objects, bucket) {
...
export function snapshotUsage(snapshot) {
    return snapshot.objects.reduce((usage, object) => ({
        bytes: usage.bytes + (object.data?.byteLength ?? 0),
        objects: usage.objects + 1,
    }), { bytes: 0, objects: 0 });
}
export function emptySnapshot(generation) {
    return {
        formatVersion: BROWSER_PERSISTENCE_FORMAT_VERSION,
        generation,
        buckets: [],
...
function normalizePutBucket(type, value) {
...
function normalizeDeleteBucket(type, value) {
...
function normalizeDeleteObject(type, value) {
...
//# sourceMappingURL=persistence-format.js.map