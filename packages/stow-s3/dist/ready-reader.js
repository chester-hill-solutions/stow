...
import { parseReadyMessage, READY_PROTOCOL_VERSION, StowProtocolError } from "./ready.js";
...
export async function waitForReady(child, timeoutMs = 10_000, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onReadyData = (chunk) => {
            ...
            if (readyBuffer.length > MAX_READY_RECORD_BYTES) {
                fail(new StowProtocolError("protocol_mismatch", `readiness record exceeded ${MAX_READY_RECORD_BYTES} bytes without a newline`));
                return;
            }
        ...
        const onReadyEnd = () => {
            endGraceTimer = setTimeout(() => {
                fail(new Error(`readiness descriptor closed before a complete STOW_READY record arrived.\nstdout:\n${stdoutBuffer}\nstderr:\n${stderrBuffer}`));
            }, READY_END_GRACE_MS);
        };
        const onReadyError = (error) => {
            fail(new Error(`readiness descriptor failed: ${error.message}\nstdout:\n${stdoutBuffer}\nstderr:\n${stderrBuffer}`));
        };
...
//# sourceMappingURL=ready-reader.js.map