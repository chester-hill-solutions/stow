...
export function validateQuotaOptions(options: EmbeddedStowOptions): void {
...
export function validateSnapshotQuota(snapshot: PersistenceSnapshot, options: EmbeddedStowOptions): void {
...
export function verifyRestored(embedded: EmbeddedStow, snapshot: PersistenceSnapshot): void {
...
export function restoreSnapshot(embedded: EmbeddedStow, snapshot: PersistenceSnapshot): void {
  embedded.reset();
...
export function normalizePersistenceError(error: unknown): BrowserPersistenceError {
...
function isStableCode(value: unknown): value is BrowserPersistenceError["code"] {
  return (
    value === "persistence_unsupported" ||
