...
import {
  platformPackageForCurrentPlatform,
  resolveStowBinaryDetailed,
  stowBinaryAvailable,
  type StowBinarySource,
} from "./bin.js";
import { READY_PROTOCOL_VERSION } from "./ready.js";

const requireFromHere = createRequire(import.meta.url);

const packageVersion: string = (requireFromHere("../package.json") as { version: string }).version;
...
function clientChecks(): DoctorCheck[] {
  const resolved = resolveStowBinaryDetailed();
  const available = stowBinaryAvailable();
  const resolution: DoctorCheck = available
    ? succeeded(
        "client.binary.resolution",
        true,
        `${resolved.path} (from ${SOURCE_DESCRIPTIONS[resolved.source]})`,
      )
    : failed(
        "client.binary.resolution",
        true,
        "no runnable stow binary was found",
        resolved.path,
      );
  ...
  const platformCheck: DoctorCheck =
    expectedPackage === undefined
      ? failed(
          ...
          false,
          `no stow binary is published for ${process.platform}-${process.arch}`,
        )
      : succeeded(
          "client.platform.package",
          false,
          resolved.source === "platform-package"
 ...
 */
function runBinaryDoctor(
  binary: string,
  timeoutMs: number,
): Promise<{ report?: BinaryReport; error?: string }> {
  return new Promise((resolve) => {
    execFile(
      binary,
      ["doctor", "--json"],
      { timeout: timeoutMs, maxBuffer: MAX_DIAGNOSTIC_BYTES, encoding: "utf8" },
      (error, stdout, stderr) => {
        if (error !== null) {
        ...
        try {
          resolve({ report: JSON.parse(stdout) as BinaryReport });
        } catch (parseError) {
          resolve({
            error: `"${binary} doctor --json" did not return JSON: ${
...
function versionCheck(binaryVersion: string | undefined): DoctorCheck {
  ...
  return failed(
    ...
    false,
    `this package is ${packageVersion} but the binary is ${binaryVersion}`,
  );
}
...
export async function runDoctor(options: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  const resolved = resolveStowBinaryDetailed();
  const checks: DoctorCheck[] = clientChecks();
  ...
  if (stowBinaryAvailable()) {
    ...
    checks.push(
      failed(
        "client.binary.doctor",
        true,
        "skipped because no runnable stow binary was found",
      ),
    );
  }
