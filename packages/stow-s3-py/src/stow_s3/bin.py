...
BUNDLED_SUBPATH = Path("_bin") / "stow-s3"
...
class StowBinaryNotFoundError(Exception):
    ...
    def __init__(self, resolved: str) -> None:
        super().__init__(
            f"The stow-s3 server binary was not found (resolved to {resolved!r}). "
            "It is searched for in this order: the platform wheel that ships it "
            "inside this package, the STOW_BIN environment variable, bin/stow-s3 "
            "relative to a monorepo checkout, then stow-s3 on PATH. Install stow-s3 "
...
def _monorepo_binary() -> Path | None:
    ...
    for _ in range(MONOREPO_DEPTH):
        if (directory / "go.mod").is_file():
            candidate = directory / "bin" / "stow-s3"
...
def resolve_stow_binary_detailed() -> ResolvedBinary:
    ...
    return ResolvedBinary("stow-s3", BinarySource.PATH)
...
def stow_binary_available() -> bool:
    ...
    if resolved.source is BinarySource.PATH:
        return shutil.which("stow-s3") is not None
...
def describe_source(source: BinarySource) -> str:
    ...
    return {
        ...
        BinarySource.MONOREPO: "bin/stow-s3 in the monorepo checkout",
