...
(`@chs/stow-<os>-<arch>`) resolved by the package before `PATH`, so a plain
install starts a session with no environment setup. The resolution order is:
bundled platform package, `STOW_BIN`, monorepo `bin/stow`, `stow` on `PATH`.
...
- **Embedded S3 wire compatibility.** The native path has an in-process adapter;
  the public `EmbeddedStow` and `@chs/stow/browser` profiles stay direct object
